...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def compute_kl_related_metrics(self, batch: DataProto, metrics: dict, timing_raw: dict):
        ...
        with marked_timer("old_log_prob", timing_raw, "blue"):
            old_log_prob = self.actor_rollout_wg.compute_log_prob_h(batch)
            #W = old_log_prob.meta_info['W']
            #b = old_log_prob.meta_info['b']
            pred = old_log_prob.batch['pred']
            entropys = old_log_prob.batch["entropys"]
            weights = old_log_prob.batch["weights"]
            weighted_adv = old_log_prob.batch["weighted_adv"]
            advantages = batch.batch['advantages']


            response_masks = batch.batch["response_mask"]

            weighted_mask = weights*response_masks
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    #rewards_a = batch.batch["token_level_rewards"]  # 形状: [BS, Len]
                    #a = rewards_a.sum(dim=1, keepdim=True).expand_as(rewards_a).clone()
                    #print(a)
                    #exit(2333)

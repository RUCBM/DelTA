...
def calculate_rhoed_adv(score, response_mask, advantage, c=1.0, eps=1e-6):
    """
    Symmetric soft-clip reliability gate.

    Args:
        score: [B, S] tensor.
            Discriminative score, e.g. score = W^T v + b.
        response_mask: [B, S] tensor.
            1 = valid response token, 0 = non-response / padding.
        advantage: [B, S] or [B] tensor.
            Token-level or sequence-level advantage.
            - If [B], it will be broadcast to [B, S].
        c: float.
            Soft-clip width. Default 1.0.
            Gate is:
                rho = clip((m_hat + c) / (2c), 0, 1)
        eps: float.
            Numerical stability term.

    Returns:
        rhoed_adv: [B, S] tensor.
            Gated advantage.
        rho: [B, S] tensor.
            Reliability gate in [0, 1].
        m_hat: [B, S] tensor.
            Standardized signed margin.
    """
    ...
    if advantage.dim() == 1:
        # [B] -> [B, 1] -> [B, S]
        advantage = advantage.float().unsqueeze(1).expand_as(score)
    else:
    ...
    # -------- compute std only on valid response tokens --------
    valid_margin = margin[response_mask > 0]

    if valid_margin.numel() == 0:
        # no valid response token, return zeros safely
        rho = torch.zeros_like(score)
        rhoed_adv = torch.zeros_like(score)
        m_hat = torch.zeros_like(score)
        return rhoed_adv, rho, m_hat

    std_m = torch.std(valid_margin, unbiased=False)
    std_m = torch.clamp(std_m, min=eps)

    # -------- standardize --------
    m_hat = margin / std_m

    # -------- symmetric soft clip gate --------
    # rho = clip((m_hat + c) / (2c), 0, 1)
    rho = torch.clamp((m_hat + c) / (2.0 * c), min=0.0, max=1.0)

    # zero out non-response tokens
    rho = rho * response_mask

    # -------- gated advantage --------
    rhoed_adv = rho * advantage

    return rhoed_adv

def get_caculate_rho(score,response_mask,advantage,c=1.0):
    """
    Args:
        score: [B, S] tensor of token entropies.
        response_mask: [B, S] tensor (1 = response token, 0 = non-response).
        advantage: [B,S] tensor of token-level advantage.
        c: ratio.
        
    Returns:
        rhoed_adv: [B, S] 
    """
    score = score*response_mask
    st_m = std(score)
    score = score/(st_m+lamb)
    rho = clip((score+c)/2,0,1)
    adv = rho*advantage
    return adv

def get_global_score_top_mask(score, response_mask, top_ratio=0.8):
    """
    Select the top `top_score` high-score tokens among all response tokens in a batch.
    
    Args:
        pred: [B, response_length] tensor of token entropies.
        response_mask: [B, S] tensor (1 = response token, 0 = non-response).
    ...
    response_score = flat_score[flat_mask]
    if response_score.numel() == 0:
        return torch.zeros_like(score, dtype=torch.long)

    # Top-k selection
    top_k = max(1, int(len(response_score) * top_ratio + 0.9999)) # ceil
    _, topk_idx = torch.topk(response_score, k=top_k)
    ...
    # Build mask
    flat_out = torch.zeros_like(flat_score, dtype=torch.long)
    flat_out[top_positions] = 1
    
    return flat_out.view_as(score)  
...
def agg_loss(
    ...
    elif loss_agg_mode == "weighted-mean":
        weighted_mask = loss_mask*weights
        batch_del_sum = weighted_mask.sum().clamp_min(1e-8)
        loss = verl_F.masked_sum(loss_mat, loss_mask) / batch_del_sum * dp_size
    else:
...
def compute_policy_loss_vanilla(
    ...
    score_top_mask: torch.Tensor | None = None,
) -> tuple[torch.Tensor, dict[str, Any]]:
    """
    Compute the clipped policy objective and related metrics for PPO.
    ...
    if score_top_mask is None:
        pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode,weights=weights,**config.global_batch_info)
    else:
        pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask * score_top_mask, loss_agg_mode=loss_agg_mode,**config.global_batch_info)
